...
import CareerMotion from "../components/CareerMotion";

function useTilt(){
  const [style,setStyle]=useState({transform:"perspective(800px) rotateX(0deg) rotateY(0deg) translateY(0)"});
  const onMouseMove=(e)=>{const r=e.currentTarget.getBoundingClientRect();const x=e.clientX-r.left;const y=e.clientY-r.top;const rx=-((y/r.height-0.5)*6);const ry=(x/r.width-0.5)*6;setStyle({transform:`perspective(800px) rotateX(${rx}deg) rotateY(${ry}deg) translateY(-2px)`});};
...
function ProjectCard({item}){
  const tilt=useTilt();
  return (<a href={item.link} target="_blank" rel="noreferrer" className="block"><Card className="overflow-hidden group border-border/70 hover:border-foreground/30 transition-colors" style={tilt.style} onMouseMove={tilt.onMouseMove} onMouseLeave={tilt.onMouseLeave}><div className="relative aspect-[4/3] overflow-hidden"><img src={item.image} alt={item.title} className="w-full h-full object-cover filter grayscale group-hover:grayscale-0 transition-[filter,transform] duration-500 group-hover:scale-[1.03]" /><div className="absolute inset-0 pointer-events-none bg-gradient-to-t from-black/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" /></div><CardHeader className="pb-2"><CardTitle className="flex items-start justify-between gap-3 text-base"><span className="tracking-tight">{item.title}</span><ArrowUpRight size={18} className="opacity-60 group-hover:opacity-100" /></CardTitle></CardHeader><CardContent className="pt-0 pb-4"><p className="text-sm text-muted-foreground">{item.subtitle}</p><div className="flex flex-wrap gap-2 mt-3">{item.tags.map((t)=>(<Badge key={t} variant="secondary">{t}</Badge>))}</div><div className="flex gap-4 mt-4 text-xs text-muted-foreground">{item.impact.map((m)=>(<div key={m.label} className="flex items-center gap-1"><span className="font-medium text-foreground">{m.value}</span> {m.label}</div>))}</div></CardContent></Card></a>);
}
...
export default function Home(){
  ...
  useEffect(()=>{const onScroll=()=>{const y=window.scrollY;if(portraitRef.current) portraitRef.current.style.transform=`translateY(${Math.min(12,y*0.04)}px)`;};window.addEventListener("scroll",onScroll);return()=>window.removeEventListener("scroll",onScroll);},[]);
  ...
  return (
    <div className="py-12" id="top">
      <section className="pt-8 md:pt-14 relative">
        ...
        <div className="grid md:grid-cols-[1.2fr_0.8fr] gap-10 items-center">
          <div>
            ...
            <div className="mt-6 flex gap-3"><a href="#work"><Button>View Work</Button></a><Button variant="secondary" onClick={()=>{navigator.clipboard.writeText(profile.email);toast({title:"Email copied",description:profile.email});}}>Copy Email</Button></div>
          </div>
          <div className="relative">
            <div ref={portraitRef} className="glass rounded-xl overflow-hidden shadow-sm"><img src={profile.headshot} alt="Saksham Kalra" className="w-full h-[260px] object-cover" /></div>
            <div className="mt-4 glass rounded-xl overflow-hidden shadow-sm"><div className="aspect-video"><video src={profile.videoMp4} muted loop autoPlay playsInline className="w-full h-full object-cover" /></div></div>
          </div>
        </div>
      </section>
      ...
      <section id="downloads" className="mt-20"><div className="flex items-end justify-between"><h3 className="font-display text-2xl md:text-3xl tracking-tight">Case Studies & Decks</h3><a href="#top" className="text-sm opacity-80 hover:opacity-100">Back to top</a></div><div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">{downloads.map((d)=>(<a key={d.label} href={d.href} target="_blank" rel="noreferrer" className="block"><Card className="hover:border-foreground/40 transition-colors"><CardContent className="p-5 flex items-center justify-between"><span className="text-sm">{d.label}</span><Download size={18} className="opacity-70" /></CardContent></Card></a>))}</div></section>
